...
@export var use_input_emit := true
...
func _update_joystick(touch_position: Vector2) -> void:
    ...
    if vector.length_squared() > deadzone_size * deadzone_size:
        ...
        is_pressed = false
        output = Vector2.ZERO
    var ax = abs(output.y) < abs(output.x)
    var ay = abs(output.y) > abs(output.x)
    if use_input_actions:
        if output.x > 0 and abs(output.y) < abs(output.x):
            Input.action_release(action_left)
            Input.action_press(action_right, output.x)
        elif output.x < 0 and abs(output.y) < abs(output.x):
            Input.action_release(action_right)
            Input.action_press(action_left, -output.x)

        if output.y > 0 and abs(output.y) > abs(output.x):
            Input.action_release(action_up)
            Input.action_press(action_down, output.y)
        elif output.y < 0 and abs(output.y) > abs(output.x):
