...
func _load_editor_config():
    var editor_config_path = GutEditorGlobals.editor_run_gut_config_path
    if(FileAccess.file_exists(editor_config_path)):
        _gut_config.load_options(editor_config_path)
    elif(FileAccess.file_exists('res://.gutconfig.json')):
        # Let repository config seed the editor panel on first run.  GUT's CLI
        # reads res://.gutconfig.json by default, but the editor panel stores its
        # mutable config in user://gut_temp_directory/gut_editor_config.json.
        _gut_config.load_options('res://.gutconfig.json')
    else:
        _gut_config.load_options(editor_config_path)
