...
func _make_info(editor, script, test_script_flag):
    ...
    while(line > 0 and (!done_func or !done_inner)):
        # Avoid CodeEdit.can_fold_line() here.  In Godot 4.7 the editor can
        # briefly report an invalid folding gutter index while scripts/plugins
        # are still initializing, which logs:
        #   TextEdit: Index p_gutter = -1 is out of bounds
        # We only need to scan upward for func/class declarations, so reading the
        # line text directly is both sufficient and avoids touching gutters.
