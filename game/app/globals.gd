...
var D = { init_pos = Quaternion(0, 0.707, 0, 0.707).normalized() }
var data := { }
## has to be empty
var settings := { }
...
func _ready() -> void:
    ...
    if saved.load(PROGRESS_PATH) == OK:
        ...
        unlocked_difficulty = clampi(
            int(saved.get_value("progress", "unlocked_difficulty", 0)),
            0,
            UpgradeCatalog.data.difficulty_levels.size(),
        )
...
func discover_modifier(id: String) -> void:
    if discovered_modifiers.has(id) or not UpgradeCatalog.data.pickups.any(
            func(entry):
                return entry.id == id,
        ):
        return
