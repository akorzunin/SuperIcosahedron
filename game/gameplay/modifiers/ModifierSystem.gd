...
func collect(session: Object, pickup: ModifierData) -> void:
    ...
    if not all_in and forge_slots > 0 and pickup.pickup_kind in ["points", "tier"] \
        ...
        discard_forge()
    var message := ""
    var repeats := 2 if echo_pending and pickup.pickup_kind in ["points", "tier"] else 1
    if pickup.pickup_kind in ["points", "tier"]:
        echo_pending = false
    match pickup.pickup_kind:
        "points":
            ...
            accumulated_points += int(UpgradeCatalog.data.points_by_tier[tier - 1]) * crafted_multiplier * points_multiplier * repeats
...
func chain_summary() -> String:
    ...
    if not pending:
        return "%s score immediately · TIER upgrades next POINTS%s" % [
            title,
