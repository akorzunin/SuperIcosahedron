...
func test_all_in_requires_points_on_next_shell() -> void:
    for next_id in ["points", "tier", "echo", "inversion", "all_in", ""]:
        ...
        assert_eq(run.score, 1000 if next_id == "points" else 0)
        assert_eq(run.modifier_system.pending, next_id == "tier")
...
func test_pickup_messages_describe_effect_and_use_config_color() -> void:
    ...
    assert_eq(messages[0], "Tier armed · 2 POINTS remaining · +0 pending")
    assert_eq(messages[1], "Points charged · +250 pending")
    assert_eq(messages[2], "Tier armed · 2 POINTS remaining · +250 pending")
    assert_eq(
        messages[3],
        "Sign → negative · Next POINTS ×2 · Convert before banking · -250 pending",
    )
    assert_eq(messages[4], "Sign → positive · Bonus kept · +250 pending")
    assert_eq(messages[5], "Points charged · +750 pending")
        ...
        for j in 20:
            ...
            if side.modifier:
                ...
                if pickup.pickup_kind == "tier":
                    tiers += 1
                if i == 0:
                    assert_has(["points", "tier"], pickup.pickup_kind)
            else:
...
func test_far_pickups_grant_stronger_base_and_tier_values() -> void:
    ...
    assert_eq(run.modifier_system.tier_remaining, 3)
