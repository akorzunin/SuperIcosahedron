...
class ExportTests(unittest.TestCase):
    def setUp(self):
        ...
        for directory in (
            "game/app",
            "game/services/discord",
            "addons/discord-rpc-gd/bin",
            "addons/gut",
            "dev",
            "test",
        ):
        ...
        (self.source / "game/app/version.gd").write_text(
            'const VERSION = "__VERSION__"\n'
        )
        (self.source / "game/services/discord/DiscordStatus.gd").write_text(
            "DiscordRPC.refresh()\n"
        )
    ...
    def test_platform_isolation(self):
        ...
        for target in ("linux", "windows", "web", "android", "test"):
            with self.subTest(target=target):
                ...
                self.assertEqual(
                    "DiscordRPCLoader" in (work / "project.godot").read_text(), desktop
                )
                self.assertEqual(
                    (work / "game/services/discord/DiscordStatus.gd").exists(), desktop
                )
                self.assertEqual((work / "test").exists(), target == "test")
                self.assertEqual((work / "dev").exists(), target == "test")
                self.assertIn(
                    "enabled=PackedStringArray()", (work / "project.godot").read_text()
                )
                self.assertIn('v1.0\\"test', (work / "game/app/version.gd").read_text())
                self.assertIn(
                    "DISCORD_APP_ID = 123", (work / "game/app/env.gd").read_text()
                )
                self.assertEqual(
                    "use_gradle_build=false"
                    in (work / "export_presets.cfg").read_text(),
                    target == "android",
                )
